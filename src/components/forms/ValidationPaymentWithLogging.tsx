...
const ValidationPaymentWithLogging: React.FC<ValidationPaymentProps> = ({ 
  onboardingData, 
  onBack, 
  onSuccess 
}) => {
  ...
  useEffect(() => {
    errorLogger.info('ValidationPaymentWithLogging component mounted', {
    ...
    errorLogger.logIPhone15ProMaxIssue('ValidationPayment Mounted', {
      componentState: 'initialized',
      onboardingData: onboardingData ? Object.keys(onboardingData) : []
    });

    return () => {
      errorLogger.info('ValidationPaymentWithLogging component unmounted');
    };
  }, [onboardingData]);

  const handleBackClick = () => {
    errorLogger.info('ValidationPayment: User clicked back button');
  ...
  const handlePaymentError = (errorMessage: string) => {
    ...
    errorLogger.error('ValidationPayment: Payment error occurred', { error: errorMessage });
    
    // Special logging for iPhone 15 Pro Max
    errorLogger.logIPhone15ProMaxIssue('Payment Error', { error: errorMessage });
    
    setError(errorMessage);
  };

  const handlePaymentSuccess = () => {
    errorLogger.info('ValidationPayment: Payment successful');
                ...
                <CardDescription className="text-sm sm:text-base text-muted-foreground">
                  ...
                  {isIPhone15ProMax && (
                    <div className="mt-2 px-2 py-1 bg-yellow-100 text-yellow-800 rounded text-xs">
                      iPhone 15 Pro Max - Enhanced logging enabled
                    </div>
                  )}
                </CardDescription>
              </CardHeader>
...
export default ValidationPaymentWithLogging;